...
- Date: August 13, 2026
- Baseline: `origin/main` at `ebf19f6`
...
| Receipts, reservations, deliveries, custody and immutable inventory projections                                         | `catalog_inventory.py`, `inventory_projection_service.py`, `goods_receipts.py`, delivery modules and invariant tests                                                    | Partial: customer returns/damage #56, #65–#70; Transfer and general Inventory Adjustment #104, #107 |
...
| Invoices, receipts, allocations, credits, adjustments and aging                                                         | `invoice_posting.py`, `payment_allocation.py` and `customer_statement.py` implement invoice/allocation/basic credit/statement foundations                               | Partial: durable Credit Note, overpayment, reversals, complete states and rebuild #57, #71–#76      |
...
| Return request/approval against Delivered Quantity                                                                      | No Returns runtime module, migration, API or contract test exists                                                                                                       | Missing: #56, #65                                                                                   |
...
| Import/export tools for legacy migration                                                                                | OpenAPI export is not a business migration/export tool; no legacy import pipeline or role-aware history export exists                                                   | Missing: migration #100 and customer export #96                                                     |
...
The first business slice is #65, approving a Return Authorization against
Delivered Quantity net of prior return activity. Other independent entry
slices are #71, #72, #77, #78, #83, #94, #97, and #105–#110. Delivery still
proceeds one vertical slice at a time; the additional ready issues make
blockers visible rather than inviting parallel mutation of shared domain
foundations.

## Completion rule

The overall first-release goal remains active until every required parent has
passed its functional exit criteria and #103 records business UAT, audit
sign-off, and an explicit readiness decision. The #110 baseline and final
success-measure comparison must also be approved. A closed child issue, a working
happy-path UI, or a previously green CI run is not sufficient evidence by
itself.
