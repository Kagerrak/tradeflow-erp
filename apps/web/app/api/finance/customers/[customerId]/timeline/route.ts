...
export async function GET(
  ...
  const eventType = searchParams.getAll("event_type");
    ...
    return Response.json(
      {
        ...
        message: "Sign in before viewing a customer timeline.",
      },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    return Response.json(
      {
        code: envelope.error?.code ?? "timeline_unavailable",
        correlationId: envelope.error?.correlation_id ?? correlationId,
        kind: failureKind(result.response.status),
        message: envelope.error?.message ?? "Timeline was not returned.",
      },
      { status: result.response.status },
    );
  } catch {
    return Response.json(
      {
        ...
        correlationId,
        kind: "unavailable",
        message: "Timeline service unavailable; retry unchanged work.",
      },
      { status: 503 },
    );
  }
