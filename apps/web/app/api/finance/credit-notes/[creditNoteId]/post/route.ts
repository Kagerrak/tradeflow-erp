...
export async function POST(
    ...
    return Response.json(
      {
        ...
        message: "Sign in before authorizing a credit note.",
      },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    return Response.json(
      {
        code: envelope.error?.code ?? "credit_note_post_rejected",
        correlationId: envelope.error?.correlation_id ?? correlationId,
        kind: failureKind(result.response.status),
        message:
          envelope.error?.message ?? "Credit note could not be authorized.",
      },
      { status: result.response.status },
    );
  } catch {
