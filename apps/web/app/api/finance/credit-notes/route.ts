...
export async function GET(): Promise<Response> {
    ...
    return Response.json(
      {
        ...
        message: "Sign in before viewing credit notes.",
      },
      { status: 401 },
    );
  }
  ...
  try {
    const result = await client.GET("/v1/finance/credit-notes");
    ...
    return Response.json(
      {
        code: envelope.error?.code ?? "credit_note_list_rejected",
        correlationId: envelope.error?.correlation_id ?? correlationId,
        kind: failureKind(result.response.status),
        message: envelope.error?.message ?? "Credit note list is unavailable.",
      },
      { status: result.response.status },
    );
  } catch {
