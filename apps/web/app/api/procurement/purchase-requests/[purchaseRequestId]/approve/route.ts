...
export async function POST(
  ...
  const command = (await request.json()) as { expected_version: number };
  ...
  try {
    ...
    return Response.json(
      {
        code: envelope.error?.code ?? "purchase_request_not_approved",
        correlationId: envelope.error?.correlation_id ?? correlationId,
        kind: failureKind(result.response.status),
        message:
          envelope.error?.message ?? "Purchase request was not approved.",
      },
