...
type CreateGoodsReceiptInput =
  components["schemas"]["CreateGoodsReceiptCommand"];
...
export async function POST(
  ...
  const command = (await request.json()) as CreateGoodsReceiptInput;
  ...
  try {
    ...
    return Response.json(
      {
        code: envelope.error?.code ?? "goods_receipt_not_posted",
        correlationId: envelope.error?.correlation_id ?? correlationId,
        kind: failureKind(result.response.status),
        message: envelope.error?.message ?? "Goods receipt was not posted.",
      },
    ...
    return Response.json(
      {
        code: "goods_receipt_service_unavailable",
        correlationId,
        kind: "unavailable",
